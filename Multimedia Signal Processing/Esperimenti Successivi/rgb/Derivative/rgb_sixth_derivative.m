...
% Defining filter:
% - fourth derivative (vertical and horizontal)
% - quantization
f_v = [1,-6,15,-20,15,-6,1];
f_h = f_v.';
h_o = [0,1,5,25,125];
...
for k = 1 : length(training_image)
    ...
    x = double(imread(current_image));
    features_vector_training = rgb_features_vector(x, f_v, f_h, h_o);
...
for k = 1 : length(test_image)
    ...
    x = double(imread(current_image));
    features_vector_test = rgb_features_vector(x, f_v, f_h, h_o);
